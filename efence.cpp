...
// Modified by werner.van.belle@gmail.com to selectively check specific 
// allocations
...
enum _Mode {
  ...
  INTERNAL_USE	/* A buffer used internally by efence_malloc(). */
};
...
/*
 * EF_ALLOW_MALLOC_0 is set if Electric Fence is to allow efence_malloc(0). I
 * trap efence_malloc(0) by default because it is a common source of bugs.
 */
...
static void initialize(void)
{
	 ...
	 * alignment for efence_malloc(). We want that alignment to be under
	 * user control, since smaller alignment lets us catch more bugs,
	 * however some software will break if efence_malloc() returns a buffer
	 * that is not word-aligned.
	 *
	 * I would like
	 * alignment to be zero so that we could catch all one-byte
	 * overruns, however if efence_malloc() is asked to allocate an odd-size
	...
	/*
	 * See if the user wants to allow efence_malloc(0).
	 */
	...
	/*
	 * Allocate memory, and break it up into two efence_malloc buffers. The
 ...
 * left to support the allocation of a single efence_malloc buffer.
 */
 ...
 */
void * memalign(size_t alignment, size_t userSize)
{
	register Slot *	slot;
	...
	/*
	 * If EF_PROTECT_BELOW is set, all addresses returned by efence_malloc()
 ...
 * before that buffer in the address space. This is used by efence_free() to
...
bool efence_free(void * address)
{
  ...
  if ( slot->mode != ALLOCATED ) 
	...
	EF_Abort("efence_free(%a): freeing free memory.",address);
    }
   ...
   * is true, free memory is never efence_reallocated, so it remains access
...
void * efence_realloc(void * oldBuffer, size_t newSize)
{
	...
	if ( oldBuffer ) {
		...
		/* Internal memory was re-protected in efence_free() */
	}
	unlock();
