...
public:
  ...
  /**
   * This constructor accepts store as the storage and will increase
   * the reference counter of the store with one. The sizes are set
   * to the given size_to
   */
...
void ArrayMeta<D,T>::replaceAccess(ArrayMeta<D,T> *other)
{
...
template <int D, class T> 
void ArrayMeta<D,T>::printMetaInfo()
{
...
template <int D, class T> 
T * ArrayMeta<D,T>::address(const Position<D> & c)
{
  T* result = offset;
...
template <int D, class T> 
T * ArrayMeta<D,T>::address(const From<D> & c)
{
  T* result = offset;
  iterate_dimensions(d,D,result+=c[d]*delta[d]);
  return result;
...
template <int D, class T> 
ArrayMeta<D,T>::ArrayMeta(Size<D> size)
{
  refcount = 1;
  long totsiz = setSize(size);
  NormalArrayStorage<T> * nas =  new NormalArrayStorage<T>(totsiz);
  storage = nas;
  offset = storage->data;
}
...
template <int D, class T> long ArrayMeta<D,T>::setSize(const Size<D> & s)
{
  long int totsize = 1;
  size = s;
  for(int i = D-1 ; i >=0 ; i --)
